...
int getChar(void);
int GAME_LOOP(const char *word,UINT8 bufet);
      ...
      switch(cur_var)
      {
         ...
         case LoopGame:
               ...
               while(!trials == 0)
               {
                  ...
                  puts(output4);
                  if(bufet == '#') // 255 is return -1 in ASCII
                  {
                     flag = 1;
                     cur_var = READ;
                     break;
...
{
   char time[50];
   sprintf(output3, "***DEBUG GAME_LOOP: WORD change: %s \r\n***DEBUG GAME_LOOP: Character change: %c ", word, bufet);
   puts(output3);
