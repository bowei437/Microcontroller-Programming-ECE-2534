...
void __ISR(SLAVE_SPI_VECTOR, IPL7SRS) _SPISlaveHandler(void) {
    ...
    char dataToSend = 0;
    DBG_ON(MASK_DBG2);
    receivedData = SpiChnReadC(CHANNEL_SLAVE_SPI);
    ...
    SpiChnWriteC(CHANNEL_SLAVE_SPI, dataToSend);
    DBG_OFF(MASK_DBG2);
    INTClearFlag(SLAVE_SPI_INT);
}
...
void initSlaveSPI() {
    SpiChnOpen(CHANNEL_SLAVE_SPI,
            ...
            | SPI_OPEN_ON,
            SPI_SOURCE_CLOCK_DIVIDER);
    INTSetVectorPriority(INT_SPI_4_VECTOR, INT_PRIORITY_LEVEL_7);
    INTClearFlag(SLAVE_SPI_INT);
    INTEnable(SLAVE_SPI_INT, INT_ENABLED);
}
...
void sendMasterSPIData(int request, char inData[]) {
    int i = 0;
    // Make request first
    SpiChnPutC(CHANNEL_MASTER_SPI, request);
    char dummyValue = SpiChnGetC(CHANNEL_MASTER_SPI);
    while (1) {
        SpiChnPutC(CHANNEL_MASTER_SPI, dummyValue);
        inData[i] = SpiChnGetC(CHANNEL_MASTER_SPI);
        if (inData[i] == 0) {
            return;
        }
        i++;
